...
namespace Rtsp
{
   public class UDPSocket
   {
      ...
      public UDPSocket(string data_multicast_address, int data_multicast_port, string control_multicast_address, int control_multicast_port)
      {
         ...
         // open a pair of UDP sockets - one for data (video or audio) and one for the status channel (RTCP messages)
         data_port = data_multicast_port;
         control_port = control_multicast_port;

         try {
            var data_ep = new IPEndPoint(IPAddress.Any, data_port);
            var control_ep = new IPEndPoint(IPAddress.Any, control_port);
      ...
      private readonly IPAddress control_mcast_addr;
      private Thread control_read_thread;
      private readonly UdpClient control_socket;
      private readonly IPAddress data_mcast_addr;

      private Thread data_read_thread;

      private readonly UdpClient data_socket;

      private readonly bool is_multicast;
      ...
      {
         var ipEndPoint = new IPEndPoint(IPAddress.Any, data_port);
         try {
            // loop until we get an exception eg the socket closed
            while (true) {
               var frame = socket.Receive(ref ipEndPoint);
