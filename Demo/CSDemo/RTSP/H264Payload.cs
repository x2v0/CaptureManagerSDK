   ...
   public class H264Payload
   {
      ...
      public List<byte[]> Process_H264_RTP_Packet(byte[] rtp_payload, int rtp_marker)
      {
         ...
         if (rtp_marker == 1) {
            ...
            var nal_units = Process_H264_RTP_Frame(temporary_rtp_payloads);
      ...
      private List<byte[]> Process_H264_RTP_Frame(List<byte[]> rtp_payloads)
      {
         ...
         for (var payload_index = 0; payload_index < rtp_payloads.Count; payload_index++) {
            ...
            var nal_header_f_bit = (rtp_payloads[payload_index][0] >> 7) & 0x01;
            var nal_header_nri = (rtp_payloads[payload_index][0] >> 5) & 0x03;
            var nal_header_type = (rtp_payloads[payload_index][0] >> 0) & 0x1F;

            // If the Nal Header Type is in the range 1..23 this is a normal NAL (not fragmented)
            // So write the NAL to the file
            if ((nal_header_type >= 1) &&
                (nal_header_type <= 23)) {
            ...
            // There are 4 types of Aggregation Packet (split over RTP payloads)
               ...
               var fu_header_s = (rtp_payloads[payload_index][1] >> 7) & 0x01; // start marker
               var fu_header_e = (rtp_payloads[payload_index][1] >> 6) & 0x01; // end marker
               var fu_header_r = (rtp_payloads[payload_index][1] >> 5) & 0x01; // reserved. should be 0
               var fu_header_type = (rtp_payloads[payload_index][1] >> 0) & 0x1F; // Original NAL unit header
