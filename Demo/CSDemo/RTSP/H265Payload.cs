...
namespace Rtsp
{
   ...
   public class H265Payload
   {
      ...
      private readonly bool has_donl;
      // H265 / HEVC structure.
      ...
      private int single, agg, frag; // used for diagnostics stats
      ...
      public List<byte[]> Process_H265_RTP_Packet(byte[] rtp_payload, int rtp_marker)
      {
         ...
         if (rtp_marker == 1) {
            ...
            var nal_units = Process_H265_RTP_Frame(temporary_rtp_payloads);
      ...
      private List<byte[]> Process_H265_RTP_Frame(List<byte[]> rtp_payloads)
      {
         ...
         for (var payload_index = 0; payload_index < rtp_payloads.Count; payload_index++) {
            ...
            var payload_header = (rtp_payloads[payload_index][0] << 8) | rtp_payloads[payload_index][1];
            var payload_header_f_bit = (payload_header >> 15) & 0x01;
            var payload_header_type = (payload_header >> 9) & 0x3F;
            var payload_header_layer_id = (payload_header >> 3) & 0x3F;
            var payload_header_tid = payload_header & 0x7;
            ...
            // 33=SPS
            // 34=PPS
            if ((payload_header_type != 48) &&
                (payload_header_type != 49)) {
            ...
            // Aggregation Packet
               ...
               // available data
               try {
                  var ptr = 2; // start after 16 bit Payload Header

                  // loop until the ptr has moved beyond the length of the data
                  while (ptr < (rtp_payloads[payload_index].Length - 1)) {
                     if (has_donl) {
                        ptr = ptr + 2; // step over the DONL data
                     }

                     var size = (rtp_payloads[payload_index][ptr] << 8) + (rtp_payloads[payload_index][ptr + 1] << 0);
                     ptr = ptr + 2;
                     var nal = new byte[size];
                     Array.Copy(rtp_payloads[payload_index], ptr, nal, 0, size); // copy the NAL
            ...
            // Fragmentation Unit
               ...
               var fu_header_s = (rtp_payloads[payload_index][2] >> 7) & 0x01; // start marker
               var fu_header_e = (rtp_payloads[payload_index][2] >> 6) & 0x01; // end marker
               var fu_header_type = (rtp_payloads[payload_index][2] >> 0) & 0x3F; // fu type
               ...
               if ((fu_header_s == 1) &&
                  ...
                  var nal_header = payload_header & 0x81FF; // strip out existing 'type'
