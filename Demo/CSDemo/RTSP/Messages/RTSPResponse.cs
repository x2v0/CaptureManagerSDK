...
namespace Rtsp.Messages
{
   public class RtspResponse : RtspMessage
   {
      ...
      public int Timeout
      {
         get
         {
            var returnValue = DEFAULT_TIMEOUT;
            if (Headers.ContainsKey(RtspHeaderNames.Session)) {
               var parts = Headers[RtspHeaderNames.Session].Split(';');
               if (parts.Length > 1) {
         ...
         set
         {
            if (Headers.ContainsKey(RtspHeaderNames.Session)) {
               if (value != DEFAULT_TIMEOUT) {
                  Headers[RtspHeaderNames.Session] = Headers[RtspHeaderNames.Session].Split(';').First() + ";timeout=" + value.ToString(CultureInfo.InvariantCulture);
               } else {
                  //remove timeout part
