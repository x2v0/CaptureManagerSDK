...
*/
      ...
      public LowerTransportType LowerTransport
      {
         get;
         set;
      }
      ...
      public static RtspTransport Parse(string aTransportString)
      {
         ...
         var transportPart = aTransportString.Split(';');
         var transportProtocolPart = transportPart[0].Split('/');
         ...
         foreach (var part in transportPart) {
            ...
            switch (subPart[0].ToUpperInvariant()) {
               ...
               case "TTL":
                  var ttl = 0;
                  if ((subPart.Length < 2) ||
                      !int.TryParse(subPart[1], out ttl)) {
               ...
               case "LAYERS":
                  var layers = 0;
                  if ((subPart.Length < 2) ||
                      !int.TryParse(subPart[1], out layers)) {
      ...
      public override string ToString()
      {
         ...
         if (ClientPort != null) {
            ...
            transportString.Append(ClientPort);
         }

         if (ServerPort != null) {
            transportString.Append(";server_port=");
            transportString.Append(ServerPort);
         }
      ...
      {
         ProfileType profile;
         if ((transportProtocolPart.Length < 2) ||
             !Enum.TryParse(transportProtocolPart[1], out profile)) {
