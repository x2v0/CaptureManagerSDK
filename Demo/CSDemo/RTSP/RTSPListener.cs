...
namespace Rtsp
{
   ...
   public class RtspListener : IDisposable
   {
      ...
      public IAsyncResult BeginSendData(int channel, byte[] frame, AsyncCallback asyncCallback, object state)
      {
         ...
         var data = new byte[4 + frame.Length]; // add 4 bytes for the header
         data[0] = 36; // '$' character
         data[1] = (byte) channel;
      ...
      public RtspChunk ReadOneMessage(Stream commandStream)
      {
         ...
         var currentReadingState = ReadingState.NewCommand;
               ...
               while (needMoreChar) {
                  ...
                  switch (currentByte) {
                     ...
                     case '$': // if first caracter of packet is $ it is an interleaved data packet
                        if ((currentReadingState == ReadingState.NewCommand) &&
                            (buffer.Count == 0)) {
            ...
            switch (currentReadingState) {
               ...
               case ReadingState.Data:
                  if (currentMessage.Data.Length > 0) {
                     ...
                     var byteCount = commandStream.Read(currentMessage.Data, byteReaden, currentMessage.Data.Length - byteReaden);
                     if (byteCount <= 0) {
                        currentReadingState = ReadingState.End;
                        break;
                     }
               ...
               case ReadingState.InterleavedData:
                  ...
                  var channelByte = commandStream.ReadByte();
                  if (channelByte == -1) {
                     currentReadingState = ReadingState.End;
                     break;
                  }
                  ...
                  var sizeByte1 = commandStream.ReadByte();
                  if (sizeByte1 == -1) {
                     currentReadingState = ReadingState.End;
                     break;
                  }

                  var sizeByte2 = commandStream.ReadByte();
                  if (sizeByte2 == -1) {
                     currentReadingState = ReadingState.End;
                     break;
                  }
               ...
               {
                  var byteCount = commandStream.Read(currentMessage.Data, byteReaden, size - byteReaden);
                  if (byteCount <= 0) {
                     currentReadingState = ReadingState.End;
                     break;
                  }
      ...
      private void DoJob()
      {
         try {
            ...
            while (_transport.Connected) {
               ...
               var currentMessage = ReadOneMessage(_stream);
               ...
               if (currentMessage != null) {
                  ...
                  if (currentMessage is RtspResponse) {
                     var response = currentMessage as RtspResponse;
                     lock (_sentMessage) {
