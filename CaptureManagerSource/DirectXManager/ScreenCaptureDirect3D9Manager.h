...
#define LOG_INVOKE_DX9_FUNCTION(Function, ...) lresult = ScreenCaptureDirect3D9Manager::Function(__VA_ARGS__);\
	if (FAILED(lresult))\
				{\
...
#define LOG_INVOKE_DX9_FUNCTION(Function, ...) {\
	lresult = ScreenCaptureDirect3D9Manager::Function(__VA_ARGS__);\
	if (FAILED(lresult))\
				{\
		break;\
			}\
...
#endif
#ifdef _DEBUG_CAPTUREMANAGER
#define LOG_INVOKE_DX9_METHOD(Function, ...) lresult = ScreenCaptureDirect3D9Manager::Function(__VA_ARGS__);\
	if (FAILED(lresult))\
					{\
	LogPrintOut::getInstance().printOutln(\
		LogPrintOut::ERROR_LEVEL,\
...
#define LOG_INVOKE_DX9_METHOD(Function, Object, ...) lresult = Object->Function(__VA_ARGS__);\
	if (FAILED(lresult))break;\
         ...
         class ScreenCaptureDirect3D9Manager
         {
         public:
         ...
         private:
            ...
            ScreenCaptureDirect3D9Manager(const ScreenCaptureDirect3D9Manager&) = delete;

            ScreenCaptureDirect3D9Manager& operator=(const ScreenCaptureDirect3D9Manager&) = delete;
         };
      }
   }
}
