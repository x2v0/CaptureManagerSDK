...
namespace CaptureManager
{
   namespace COMServer
   {
      class SampleGrabberCallSinkFactory : public BaseDispatch<ISampleGrabberCallSinkFactory, IContainer>
      {
      public:
         SampleGrabberCallSinkFactory();

         static GUIDToNamePair getGUIDToNamePair(); // ISampleGrabberCallSinkFactory interface
         STDMETHOD(createOutputNode)(REFGUID aRefMajorType, REFGUID aRefSubType, DWORD aSampleByteSize, REFIID aREFIID,
                                     IUnknown** aPtrPtrISampleGrabberCall) override; // IDispatch interface stub
