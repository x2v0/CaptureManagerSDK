...
CBaseReferenceClock::~CBaseReferenceClock()
{
    ...
    if (m_TimerResolution) CaptureManager::Core::Winmm::WinmmManager::timeEndPeriod(m_TimerResolution);
...
, m_hThread(0)
{
    ...
    else
    {
        ...
        TIMECAPS tc;
		m_TimerResolution = (TIMERR_NOERROR == CaptureManager::Core::Winmm::WinmmManager::timeGetDevCaps(&tc, sizeof(tc)))
                            ? tc.wPeriodMin
                            : 1;

        CaptureManager::Core::Winmm::WinmmManager::timeBeginPeriod(m_TimerResolution);

        /* Initialise our system times - the derived clock should set the right values */
		m_dwPrevSystemTime = CaptureManager::Core::Winmm::WinmmManager::timeGetTime();
...
REFERENCE_TIME CBaseReferenceClock::GetPrivateTime()
{
	...
	DWORD dwTime = CaptureManager::Core::Winmm::WinmmManager::timeGetTime();
    {
    ...
    )
{
    ...
    if( 0 == timerResolution  ) {
        if( m_TimerResolution ) {
           CaptureManager::Core::Winmm::WinmmManager::timeEndPeriod( m_TimerResolution );
           m_TimerResolution = 0;
        }
    } else {
        TIMECAPS tc;
		DWORD dwMinResolution = (TIMERR_NOERROR == CaptureManager::Core::Winmm::WinmmManager::timeGetDevCaps(&tc, sizeof(tc)))
                            ? tc.wPeriodMin
                            : 1;
