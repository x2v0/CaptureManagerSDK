...
#include "../BaseClasses.h" // DirectShow base class definitions
#include <mmsystem.h>       // Needed for definition of CaptureManager::Core::Winmm::WinmmManager::timeGetTime
...
void CBaseRenderer::TimerCallback()
{
    ...
    //  CaptureManager::Core::Winmm::WinmmManager::timeKillEvent)
    CAutoLock cRendererLock(&m_RendererLock);
...
void CBaseRenderer::ResetEndOfStreamTimer()
{
    ...
    if (m_EndOfStreamTimer) {
        CaptureManager::Core::Winmm::WinmmManager::timeKillEvent(m_EndOfStreamTimer);
        m_EndOfStreamTimer = 0;
    }
}
...
HRESULT CBaseRenderer::StartStreaming()
{
    ...
    CaptureManager::Core::Winmm::WinmmManager::timeBeginPeriod(1);
    OnStartStreaming();
...
HRESULT CBaseRenderer::StopStreaming()
{
    ...
    if (m_bStreaming == TRUE) {
        m_bStreaming = FALSE;
        OnStopStreaming();
        CaptureManager::Core::Winmm::WinmmManager::timeEndPeriod(1);
    }
    return NOERROR;
}
...
// We must do this because we call CaptureManager::Core::Winmm::WinmmManager::timeKillEvent, our timer callback method
// has to take the renderer lock to serialise our state. Therefore holding a
// renderer lock when calling CaptureManager::Core::Winmm::WinmmManager::timeKillEvent could cause a deadlock condition
...
//
// We use CaptureManager::Core::Winmm::WinmmManager::timeGetTime to return the timing counts we use (since it's relative
...
// calling CaptureManager::Core::Winmm::WinmmManager::timeBeginPeriod/CaptureManager::Core::Winmm::WinmmManager::timeEndPeriod when it changes streaming states
...
HRESULT CBaseVideoRenderer::ResetStreamingTimes()
{
    ...
    m_tStreamingStart = CaptureManager::Core::Winmm::WinmmManager::timeGetTime();
    m_trRenderAvg = 0;
...
{
    m_tStreamingStart = CaptureManager::Core::Winmm::WinmmManager::timeGetTime()-m_tStreamingStart;
    return NOERROR;
} // OnStopStreaming
...
void CBaseVideoRenderer::OnWaitEnd()
{
    ...
    // This is wrong really because CaptureManager::Core::Winmm::WinmmManager::timeGetTime() can wrap but it's
    // only for PERF
    REFERENCE_TIME tr = CaptureManager::Core::Winmm::WinmmManager::timeGetTime()*10000;
...
{
    RecordFrameLateness(m_trLate, m_trFrame);
    m_tRenderStart = CaptureManager::Core::Winmm::WinmmManager::timeGetTime();
} // OnRenderStart
...
void CBaseVideoRenderer::OnRenderEnd(IMediaSample *pMediaSample)
{
    ...
    int tr = (CaptureManager::Core::Winmm::WinmmManager::timeGetTime() - m_tRenderStart)*10000;   // convert mSec->UNITS
                                                ...
                                                __inout REFERENCE_TIME *ptrEnd)
{
    ...
    // Remember the offset from CaptureManager::Core::Winmm::WinmmManager::timeGetTime and use that.
    // This overflows all over the place, but when we subtract to get
    // differences the overflows all cancel out.
    m_llTimeOffset = trRealStream-CaptureManager::Core::Winmm::WinmmManager::timeGetTime()*10000;
#endif
    trRealStream -= m_tStart;     // convert to stream time (this is a reftime)
...
STDMETHODIMP CBaseVideoRenderer::get_AvgFrameRate( int *piAvgFrameRate)
{
    ...
    int t;
    if (m_bStreaming) {
        t = CaptureManager::Core::Winmm::WinmmManager::timeGetTime()-m_tStreamingStart;
    } else {
        t = m_tStreamingStart;
    }
